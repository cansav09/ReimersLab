...
#### Downloads the Soft Files, GPL files, and Matrix Data (or Raw files) for a list of GSE accession numbers
#### Create directories for each project and a meta data file that describes the basic info for all the included datasets
...
GSE.list=unique(as.character(t(read.csv("InflammatorySignatureDatasetsGSEs.csv")[,1])))# Load in a list of desired GSE's
GSE.list=grep("GSE",GSE.list,value=TRUE) # Keep only the GSE's and not the Array Express numbers
...
setwd("/Users/cansav091/Desktop/Neurogenomics Data/MetaAnalysisM1")
write.csv(soft.info,file="Meta Dataset Info Table.csv",quote=FALSE)
...
setwd("/Users/cansav091/Desktop/Neurogenomics Data/MetaAnalysisM1")
meta.gse=unique(read.csv("Meta Dataset Info Table.csv"))
...
save.image(file = "MetaAnalysisM1.RData")
