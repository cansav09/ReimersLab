...
gene.names=EZgenes[match(synapse.gene.names,EZgenes$symbol),] #This tells you in order of the queried entries,what the coordinates are of the second group of entries only  the first match is given 
...
for(ii in 1:nrow(gene.names)){
  ...
  SynGOterms=rbind(SynGOterms,xx)
}
setwd(outDir)
SynGOterms=SynGOterms[which(SynGOterms$Ontology=="BP"),]## selects only the BP processes
SynGOterms.info=GOterms.info[match(SynGOterms$go_id,names(GOterms.info))]
write.csv(SynGOterms,file="GO Ontology for Synapse Genes.csv")
SynGOfunctions=c()
for(ii in 1:length(SynGOterms.info)){
  SynGOfunctions=c(SynGOfunctions,SynGOterms.info[[ii]]@Term)
}
setwd(outDirGraphs)
xx=table(SynGOfunctions)
jpeg("Histogram of Synapse Gene GO Terms.jpeg",width=800)
par(mar=c(15,3,3,3),cex.lab=.4,cex.axis=.5)
barplot(sort(xx[which(xx>100)],decreasing=TRUE),las=2)
dev.off()

jpeg("Histogram of Synapse Gene GO Terms.jpeg")
par(cex.lab=.4,cex.axis=.5)
barplot(sort(table(SynGOterms$`rep(gene.names$symbol[ii], length(xx))`),decreasing=TRUE)[which(xx>100)],las=2,horiz=TRUE)
dev.off()

### How to choose which evidence is the strongest
sort(table(SynGOterms$Evidence),decreasing=TRUE)
...
rank.evidence=SynGOterms$Evidence
rank.evidence=mapvalues(rank.evidence,from = c(unique(SynGOterms$Evidence)), to=c(12,4,9,1,10,6,5,11,8,2,3,7))
rank.evidence=as.numeric(rank.evidence)
gene.GO=as.character(SynGOterms$`rep(gene.names$symbol[ii], length(xx))`)
bestGO=mapvalues(rank.evidence,from = c(12,4,9,1,10,6,5,11,8,2,3,7), to=unique(SynGOterms$Evidence))
...
for(ii in 1:length(unique(gene.GO))){
  ...
  xx=SynGOterms$go_id[xx[which(rank.evidence[xx]==min(rank.evidence[xx]))]]
  bestGO=c(bestGO,xx)
