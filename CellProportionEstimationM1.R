### Estimate propotions of different cell types in a given mixed cell type sample using Zhang data
### This is set up to estimate proportions for Gibbs et al illumina data.

#Assure in the input directory contain 'barreslab_rnaseq.csv' -
name="CellProportionEstimateM1"
...
outDirDay=paste0(mainDir,"/",as.character(Sys.Date()))
...
tmp=match(toupper(gene.name),toupper(profiles$Gene.symbol)) ### 11941 genes match
...
match(toupper(gene.name)[tmp[!is.na(tmp)]],toupper(profiles$Gene.symbol)[!is.na(tmp)])==1:11941
...
guessprops[,] = c(0.2, 0.1, 0.2, 0.5) #Assigns guess values to aforementioned matrix.
...
boxplot(t(EstPropslll2), names=Header, main='Fitted Proportions of cell-types for Stanley Brain Institute Data')
dev.off()
