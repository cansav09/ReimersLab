...
hist(gene.means[which(cell.type=="Microglia")])
...
for(ii in 1:ncol(rel.obsv.exprs)){
  samp.gene.means[ii,] <- tapply(rel.obsv.exprs[,ii],cell.type,mean) 
}
...
out.pr.gene=sort(table(profiles$Gene.Symbol..Human.[out.pts[,1]]),decreasing=TRUE)
...
rel.obsv.exprs.edit=rel.obsv.exprs[-unique(c(which(out.pr.gene>0),which(abs(gene.skew)>.7),which(abs(gene.kurt)>3))),]
cell.type.edit=cell.type[-unique(c(which(out.pr.gene>0),which(abs(gene.skew)>.7),which(abs(gene.kurt)>3)))]
