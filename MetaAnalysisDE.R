...
for(ii in 1:nrow(gpl)){
  gse.soft=paste0(soft.files,"/",gpl[ii,4])
  setwd(gse.soft)
  gse.soft=read.csv(paste0("Sample Info ",gpl[ii,3]," .csv"),skip=1,fill=TRUE,header=FALSE,colClasses="character")
  groups=rep(NA,nrow(gse.soft))
  
  headers=read.csv(paste0("Sample Info  ",gpl[ii,3]," .csv"),nrows=1,fill=TRUE,header=FALSE,colClasses="character")
...
for(ii in 1:nrow(gpl)){
  ...
  if(dir.exists(paste0(de.output,"/",gpl[ii,4]))==FALSE){
    dir.create(paste0(de.output,"/",gpl[ii,4]))
  }

  gse=gpl[ii,3]
  
  gse.dir=paste0(gse.data,"/",gpl[ii,4])
  gse.soft=paste0(soft.files,"/",gpl[ii,4])
  ...
  groups=as.character(gpl[ii,8])
  groups=as.factor(unlist(strsplit(groups,"\t")))
  if(ii==1){
    groups[grep("CN",groups)]="Control"
    groups[grep("CH",groups)]="Control Hypoxia"
    groups[grep("TN",groups)]="TLR4 KO"
    groups[grep("TH",groups)]="TLR4 KO Hypoxia"
  }
  ...
  setwd(gse.dir)
  load(paste0(gse,"-",gpl.name,"matrix.RData"))
  
  if(ii==5){
    gene.key=read.csv(paste0("GeneName-Probe",gpl.name,".csv"))
    gene.name=gene.key$x[match(geo.data$ID_REF,gene.key$X)]
    geo.data=geo.data[,grep("VALUE",colnames(geo.data))]
    geo.data=apply(geo.data[-1,],2,as.numeric)
    groups=groups[c(1,3,4,7,10,14,17,19,23,24)]
    geo.data=geo.data[,c(1,3,4,7,10,14,17,19,23,24)]
  }else{
