...
data.name="Single-cell RNA-seq of mouse hypothalamus"

############### Download SOFT file from GEO #########################
getGEOfile(gse,destdir=getwd())
gunzip(grep("soft.gz",dir(),value=TRUE))
...
# Manually type in the names of each group.
groups=c()
...
for(jj in 1:length(gpl)){
    ...
    ################ For Microarray Data #####################################
    array.type=unlist(strsplit(array.type,"\\[|\\]"))[2]## If this is microarray data it will download microarray annotation
    array.type=tolower(gsub("\\_","",array.type))
    ### Check that there is a bioconductor package for this and download it if it's not in the library 
      if(is.na(grep(paste0(array.type,".db"),installed.packages(.Library))[1])){
      if(is.na(grep(array.type,readLines("https://www.bioconductor.org/packages/3.4/data/annotation/")))[1]){
     print("There's no annotation package that is matching to this")
    }else{
      source("https://bioconductor.org/biocLite.R")
       biocLite(paste0(array.type,".db"))
      }
    }
    ...
    data.file=grep("matrix.txt$",dir(),value=TRUE)
    ...
    geo.sample.info=matrix(nrow=length(geo.data[1,]),ncol=2)
